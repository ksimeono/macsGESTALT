---
title: 'Mouse2: lineage reconstruction'
...
whitelist = read_table(whitelist_file) %>%
  ...
  filter(str_detect(cellID, '^M2_')) %>%
  mutate(cellID = str_replace(cellID, '^M2_', ''))
...
########## MANUALLY REMOVE OR FIX UP IMPERFECT CLUSTERS ###########
clusters_to_remove = c(24)
clusters_to_convert = c()
convert_to = 'WARNING'


real_clusters = real_clusters1 %>% 
  filter(!(clusterID %in% clusters_to_remove)) %>%
  mutate(clusterID = ifelse(clusterID %in% clusters_to_convert, convert_to, clusterID)) %>% 
...
* Cells are grouped by clone and within a clone, cells are arranged from most to least missing information moving left to right along the diagonal  
    ...
    - For example, the second largest clone near the top left is defined by 2 static barcodes, but most of the cells in this clone have both barcodes captured  
